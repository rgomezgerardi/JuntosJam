...
const ZD::Slate::FVector2 ScrubHandleSize(12.0f, 12.0f);
const ZD::Slate::FVector2 AlignmentMarkerSize(10.f, 20.f);
const ZD::Slate::FVector2 TextBorderSize(1.f, 1.f);
const float ScrubHandleHitBuffer = 3.0f;

class SPaperZDAnimNotifyNode;
DECLARE_DELEGATE_RetVal_FourParams(FReply, FOnNotifyNodeDragStarted, TSharedRef<SPaperZDAnimNotifyNode>, const FPointerEvent&, const ZD::Slate::FVector2&, const bool)
DECLARE_DELEGATE_RetVal_FiveParams(FReply, FOnNotifyNodesDragStarted, TArray<TSharedPtr<SPaperZDAnimNotifyNode>>, TSharedRef<SWidget>, const ZD::Slate::FVector2&, const ZD::Slate::FVector2&, const bool)
...
DECLARE_DELEGATE_TwoParams(FPanTrackRequest, int32, ZD::Slate::FVector2)
...
class SPaperZDAnimNotifyNode : public SLeafWidget
{
	...
	// End of SWidget interface
 	...
 	ENotifyStateHandleHit::Type DurationHandleHitTest(const ZD::Slate::FVector2& CursorScreenPosition) const;

	/* Cancels any drag operation. */
	void DropCancelled();

	/* Detects any overflow on the anim notify track and requests a track pan */
	float HandleOverflowPan(const ZD::Slate::FVector2& ScreenCursorPos, float TrackScreenSpaceXPosition, float TrackScreenSpaceMin, float TrackScreenSpaceMax);
	...
	bool HitTest(const FGeometry& AllottedGeometry, ZD::Slate::FVector2 MouseLocalPose) const;
	...
	void SetLastMouseDownPosition(const ZD::Slate::FVector2& CursorPosition) { MouseDownLastPosition = CursorPosition;}
	...
	/* Returns the size of this widget. */
 	const ZD::Slate::FVector2& GetSize() const { return Size; }
	...
	/* Obtain the position of this node in the parent widget. */
 	const ZD::Slate::FVector2& GetPosition() const { return Position; }
	...
	const ZD::Slate::FVector2& GetScreenPosition() const { return ScreenPosition; }
	...
	ZD::Slate::FVector2 GetNotifyPosition() const { return ZD::Slate::FVector2(NotifyTimePositionX, NotifyHeightOffset); }

	/* Offset from the widget start to the notify position. */
	ZD::Slate::FVector2 GetNotifyPositionOffset() const { return GetNotifyPosition() - GetPosition(); }
...
private:
	...
	ZD::Slate::FVector2 CachedAllotedGeometrySize;
	ZD::Slate::FVector2 Position;
	ZD::Slate::FVector2 Size;
	float NotifyTimePositionX;
	float NotifyDurationSizeX;
	float NotifyScrubHandleCentre;
	ZD::Slate::FVector2 ScreenPosition;
	ZD::Slate::FVector2 MouseDownLastPosition;
	int32 TrackIndex;
...
public:
	...
	FReply OnNotifyNodeDragStarted(TSharedRef<SPaperZDAnimNotifyNode> NotifyNode, const FPointerEvent& MouseEvent, const ZD::Slate::FVector2& ScreenNodePosition, const bool bDragOnMarker, int32 NotifyIndex);

	/* Obtain the index of the notify that was hit by the cursor, if any. */
	int32 GetHitNotifyNode(const FGeometry& MyGeometry, const ZD::Slate::FVector2& CursorPosition);
	...
	*/
	...
	FVector2D ComputeDesiredSize(float) const override
	{
 	...
 	float CalculateTime(const FGeometry& MyGeometry, ZD::Slate::FVector2 Position, bool bInputIsAbsolute = true);
...
public:
	...
	virtual void OnDragged( const class FDragDropEvent& DragDropEvent ) override
	{
		...
		ZD::Slate::FVector2 SelectionBeginPosition = TrackGeom.LocalToAbsolute(TrackGeom.AbsoluteToLocal(NodeGroupPosition) + SelectedNodes[0]->GetNotifyPositionOffset());
		...
		SelectionBeginPosition.X = TrackGeom.LocalToAbsolute(ZD::Slate::FVector2(SelectionBeginLocalPositionX, 0.0f)).X;
		...
		for (int32 NodeIdx = 0; NodeIdx < SelectedNodes.Num() && !bSnapped; ++NodeIdx)
		{
			...
			ZD::Slate::FVector2 EventPosition = SelectionBeginPosition + ZD::Slate::FVector2(TrackScaleInfo.PixelsPerInput * NodeTimeOffsets[NodeIdx], 0.0f);

			// Look for a snap on the first scrub handle
			ZD::Slate::FVector2 TrackNodePos = TrackGeom.AbsoluteToLocal(EventPosition);
			const ZD::Slate::FVector2 OriginalNodePosition = TrackNodePos;
			...
			if (!bSnapped)
			{
				ZD::Slate::FVector2 TrackNodeEndPos = TrackNodePos + CurrentNode->GetDurationSize();
		...
		CurrentDragXPosition = TrackGeom.AbsoluteToLocal(ZD::Slate::FVector2(SelectionBeginPosition.X,0.0f)).X;
		...
		if(LocalMouseXPos < LocalViewportMin && LocalViewportMin > LocalTrackMin - 10.0f)
		{
			...
			RequestTrackPan.Execute(ScreenDelta, ZD::Slate::FVector2(LocalTrackWidth, 1.f));
		}
	...
	FTrackClampInfo& GetTrackClampInfo(const ZD::Slate::FVector2 NodePos)
	{
	...
	ZD::Slate::FVector2									DragOffset;				// Offset from the mouse to place the decorator
	...
	ZD::Slate::FVector2									NodeGroupPosition;		// Position of the beginning of the selection
	ZD::Slate::FVector2									NodeGroupSize;			// Size of the entire selection
	...
	static TSharedRef<FNotifyDragDropOp> New(
		...
		const ZD::Slate::FVector2&									CursorPosition, 
		const ZD::Slate::FVector2&									SelectionScreenPosition, 
		const ZD::Slate::FVector2&									SelectionSize, 
...
{
	ZD::Slate::FVector2 ScreenNodePosition = MyGeometry.GetAbsolutePosition();
...
FReply SPaperZDAnimNotifyNode::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	float TrackScreenSpaceOrigin = CachedTrackGeometry.LocalToAbsolute(ZD::Slate::FVector2(ScaleInfo.InputToLocalX(0.0f), 0.0f)).X;
	float TrackScreenSpaceLimit = CachedTrackGeometry.LocalToAbsolute(ZD::Slate::FVector2(ScaleInfo.InputToLocalX(AnimSequence->GetTotalDuration()), 0.0f)).X;
...
ENotifyStateHandleHit::Type SPaperZDAnimNotifyNode::DurationHandleHitTest(const ZD::Slate::FVector2& CursorTrackPosition) const
{
	...
	if (NotifyDurationSizeX > 0.0f)
	{
		...
		ZD::Slate::FVector2 NotifyNodePosition(NotifyScrubHandleCentre - ScrubHandleHalfWidth, 0.0f);
		ZD::Slate::FVector2 NotifyNodeSize(NotifyDurationSizeX + ScrubHandleHalfWidth * 2.0f, NotifyHeight);

		ZD::Slate::FVector2 MouseRelativePosition(CursorTrackPosition - GetPosition());
...
float SPaperZDAnimNotifyNode::HandleOverflowPan(const ZD::Slate::FVector2& ScreenCursorPos, float TrackScreenSpaceXPosition, float TrackScreenSpaceMin, float TrackScreenSpaceMax)
{
	float Overflow = 0.0f;
...
// 	BranchingPointIconSize = ZD::Slate::FVector2(TextSize.Y, TextSize.Y);
...
// 	}
	...
	Size = bDrawTooltipToRight ? ZD::Slate::FVector2((NotifyDurationSizeX > 0.0f ? NotifyDurationSizeX : FMath::Max(LabelWidth, NotifyDurationSizeX)), NotifyHeight) : ZD::Slate::FVector2((LabelWidth + NotifyDurationSizeX), NotifyHeight);
	Size.X += NotifyHandleBoxWidth;

	if (EndMarkerNodeOverlay.IsValid())
	{
		ZD::Slate::FVector2 OverlaySize = EndMarkerNodeOverlay->GetDesiredSize();
...
FCursorReply SPaperZDAnimNotifyNode::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	...
	{
		ZD::Slate::FVector2 RelMouseLocation = MyGeometry.AbsoluteToLocal(CursorEvent.GetScreenSpacePosition());
...
{
	return FVector2D(GetSize().X, GetSize().Y);
}

bool SPaperZDAnimNotifyNode::HitTest(const FGeometry& AllottedGeometry, ZD::Slate::FVector2 MouseLocalPose) const
{
	ZD::Slate::FVector2 LocPosition = GetPosition();
	ZD::Slate::FVector2 LocSize = GetSize();
...
int32 SPaperZDAnimNotifyNode::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (EndMarkerNodeOverlay.IsValid())
	{
		ZD::Slate::FVector2 MarkerSize = EndMarkerNodeOverlay->GetDesiredSize();
		ZD::Slate::FVector2 MarkerOffset(NotifyDurationSizeX + MarkerSize.X * 0.5f + 5.0f, (NotifyHeight - MarkerSize.Y) * 0.5f);
	...
	{
		ZD::Slate::FVector2 DurationBoxSize = ZD::Slate::FVector2(NotifyDurationSizeX, TextSize.Y + TextBorderSize.Y * 2.f);
		ZD::Slate::FVector2 DurationBoxPosition = ZD::Slate::FVector2(NotifyScrubHandleCentre, (NotifyHeight - TextSize.Y) * 0.5f);
	...
	// Background
	ZD::Slate::FVector2 LabelSize = TextSize + TextBorderSize * 2.f;
	LabelSize.X += HalfScrubHandleWidth;

	ZD::Slate::FVector2 LabelPosition(bDrawTooltipToRight ? NotifyScrubHandleCentre : NotifyScrubHandleCentre - LabelSize.X, (NotifyHeight - TextSize.Y) * 0.5f);
	...
	// Text
	ZD::Slate::FVector2 TextPosition = LabelPosition + TextBorderSize;
	if (bDrawTooltipToRight)
	{
		TextPosition.X += HalfScrubHandleWidth;
	}

	ZD::Slate::FVector2 DrawTextSize;
...
{
	ZD::Slate::FVector2 ScrubHandlePosition(ScrubHandleCentre - ScrubHandleSize.X / 2.0f, (NotifyHeight - ScrubHandleSize.Y) / 2.f);
	FSlateDrawElement::MakeBox(
		OutDrawElements,
		ScrubHandleID,
...
int32 SPaperZDAnimNotifyTrack::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (TrackIndex < AnimSequence->GetNumTracks() - 1)
	{
		FSlateDrawElement::MakeLines(
			...
			TArray<ZD::Slate::FVector2>({ ZD::Slate::FVector2(0.0f, AllottedGeometry.GetLocalSize().Y), ZD::Slate::FVector2(AllottedGeometry.GetLocalSize().X, AllottedGeometry.GetLocalSize().Y) }),
			ESlateDrawEffect::None,
	...
	if (bAnyDraggedNodes && OnGetDraggedNodePos.IsBound())
	{
		...
		{
			float XPos = Value;
			TArray<ZD::Slate::FVector2> LinePoints;
			LinePoints.Add(ZD::Slate::FVector2(XPos, 0.f));
			LinePoints.Add(ZD::Slate::FVector2(XPos, AllottedGeometry.Size.Y));
...
FReply SPaperZDAnimNotifyTrack::OnNotifyNodeDragStarted(TSharedRef<SPaperZDAnimNotifyNode> NotifyNode, const FPointerEvent& MouseEvent, const ZD::Slate::FVector2& ScreenNodePosition, const bool bDragOnMarker, int32 NotifyIndex)
{
	...
	if (!bDragOnMarker)
	{
		...
		ZD::Slate::FVector2 DecoratorPosition = NodesToDrag[0]->GetPosition();
...
int32 SPaperZDAnimNotifyTrack::GetHitNotifyNode(const FGeometry& MyGeometry, const ZD::Slate::FVector2& CursorPosition)
{
...
FReply SPaperZDAnimNotifyTrack::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	{
		ZD::Slate::FVector2 CursorPos = MouseEvent.GetScreenSpacePosition();
...
float SPaperZDAnimNotifyTrack::CalculateTime(const FGeometry& MyGeometry, ZD::Slate::FVector2 Position, bool bInputIsAbsolute)
{
	if (bInputIsAbsolute)
	{
...
void SPaperZDAnimNotifyTrack::MakeNewNotifyPicker(FMenuBuilder& MenuBuilder, bool bIsReplaceWithMenu /* = false */)
{
	...
	public:
		...
		bool IsClassAllowed(const FClassViewerInitializationOptions& InInitOptions, const UClass* InClass, TSharedRef< FClassViewerFilterFuncs > InFilterFuncs) override
		{
			...
			return bChildOfObjectClass && bMatchesFlags && CastChecked<NotifyTypeClass>(InClass->GetDefaultObject())->CanBePlaced(Sequence);
		}
		...
		virtual bool IsUnloadedClassAllowed(const FClassViewerInitializationOptions& InInitOptions, const TSharedRef< const IUnloadedBlueprintData > InUnloadedClassData, TSharedRef< FClassViewerFilterFuncs > InFilterFuncs) override
		{
			...
			if (bChildOfObjectClass)
			{
				...
				{
					bValidToPlace = CastChecked<NotifyTypeClass>(NativeBaseClass->GetDefaultObject())->CanBePlaced(Sequence);
				}
			}
...
int32 SPaperZDAnimNotifyPanel::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	ZD::Slate::FVector2 Origin = AllottedGeometry.AbsoluteToLocal(Marquee.Rect.GetUpperLeft());
	ZD::Slate::FVector2 Extents = AllottedGeometry.AbsoluteToLocal(Marquee.Rect.GetSize());
...
void SPaperZDAnimNotifyPanel::RefreshMarqueeSelectedNodes(const FGeometry& PanelGeo)
{
	if (Marquee.IsValid())
	{
		...
		for (TSharedPtr<SPaperZDAnimNotifyTrack> Track : NotifyTracks)
		{
			...
			ZD::Slate::FVector2 PanelSpaceOrigin = TrackClip.GetTopLeft() - PanelClip.GetTopLeft();
			ZD::Slate::FVector2 TrackSpaceOrigin = MarqueeRect.GetTopLeft() - PanelSpaceOrigin;
...
FReply SPaperZDAnimNotifyPanel::OnNotifyNodesDragStarted(TArray<TSharedPtr<SPaperZDAnimNotifyNode>> NotifyNodes, TSharedRef<SWidget> Decorator, const ZD::Slate::FVector2& ScreenCursorPos, const ZD::Slate::FVector2& ScreenNodePosition, const bool bDragOnMarker)
{
	...
	ZD::Slate::FBox2 OverlayBounds(Nodes[0]->GetScreenPosition(), Nodes[0]->GetScreenPosition() + ZD::Slate::FVector2(Nodes[0]->GetDurationSize(), 0.0f));
	for (int32 Idx = 1; Idx < Nodes.Num(); ++Idx)
	{
		TSharedPtr<SPaperZDAnimNotifyNode> Node = Nodes[Idx];
		ZD::Slate::FVector2 NodePosition = Node->GetScreenPosition();
		float NodeDuration = Node->GetDurationSize();

		OverlayBounds += ZD::Slate::FBox2(NodePosition, NodePosition + ZD::Slate::FVector2(NodeDuration, 0.0f));
	}

	ZD::Slate::FVector2 OverlayOrigin = OverlayBounds.Min;
	ZD::Slate::FVector2 OverlayExtents = OverlayBounds.GetSize();
	
	for (TSharedPtr<SPaperZDAnimNotifyNode> Node : Nodes)
	{
		ZD::Slate::FVector2 OffsetFromFirst(Node->GetScreenPosition() - OverlayOrigin);
...
void SPaperZDAnimNotifyPanel::PanInputViewRange(int32 ScreenDelta, ZD::Slate::FVector2 ScreenViewSize)
{
